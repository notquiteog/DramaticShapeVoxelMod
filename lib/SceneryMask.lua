-- Background-connected alpha masks for native HD-2D scenery drawings.
local M={}
...
function M.mask(size,background)
 local outside,queue={},{}
 ...
 for i=0,size-1 do visit(i,0);visit(i,size-1);visit(0,i);visit(size-1,i) end
 local at=1
 while queue[at] do
 ...
 local mask={}
 for y=0,size-1 do for x=0,size-1 do mask[y*size+x]=not outside[y*size+x] end end
 return mask
end
return M
